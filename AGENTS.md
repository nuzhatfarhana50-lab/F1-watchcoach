...
- **Current phase:** Phase 3 — Canonical Watch → Learn → Connect Experience. Phases 0–2 are complete; implementation continues without discretionary pauses.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized domain contracts and live-verified Neon persistence. Phase 2 completed on 2026-08-18 with normalized Jolpica/OpenF1 adapters, saved contract payloads, typed failures, cache/request deduplication, fixture-first composition, and public race browsing through the canonical British and Dutch races.
...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Tailwind CSS 4; Vitest; React Testing Library; Playwright 1.55.1; Zod at trust boundaries; Prisma uses its PostgreSQL driver adapter and generated client; canonical seed transactions have bounded Neon-aware limits (`maxWait` 30 seconds, `timeout` 120 seconds); manual SQL checks enforce evidence ranges, standing targets, media timestamps, non-self connections, and exactly one typed external-reference target; source joins use restrictive deletion while owned records cascade; historical provider responses cache for 24 hours, recent metadata for five minutes, and live evidence for five seconds; concurrent identical provider requests share one in-flight promise; public pages consume serializable read models through a server-only composition boundary; protected media is referenced as metadata only; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run verify` passed on 2026-08-18 (Prisma validation/generation, lint, strict typecheck, 32 deterministic tests across 13 files, production build, and 2 Chromium Playwright journeys). Manual browser verification passed for `/races` → `/races/2024/12` with meaningful content, working navigation, no error overlay, and no application console errors. Live-provider smoke tests are available only through the explicit `LIVE_F1_PROVIDER_TESTS=1 npm run test:integration:live` gate and remain outside CI. Repeated live seeds and `npm run db:verify:canonical` remain green against Neon with zero invalid relations or orphaned provenance. `npm audit --omit=dev` retains the known `deepmerge-ts@7.1.5` Prisma CLI/config advisory; the offered fix is a breaking Prisma downgrade.
- **Next checkpoint:** Build `/races/[season]/[round]/moments/[moment]` around evidence-first moment detail, curated explanations, concepts, attributed media, and explicit real-moment connections; add partial-evidence, accessibility, and responsive UI coverage.
