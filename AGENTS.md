...
- **Current phase:** Phased MVP implementation complete (Phases 0–7). Deployment and hosted-integration activation remain intentionally pending explicit permission and credentials.
...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Zod at trust boundaries; Prisma/Neon as system of record; Redis REST only for ephemeral live state; authenticated mutations resolve Clerk identity to an internal `User`; all learning queries use user-scoped compound keys; live candidates use deterministic UUIDs and reuse `RaceMoment` types; AI may explain but never invent the event truth; provider fetch, normalize/cache, detect, persist, enrich, and publish are distinct Workflow checkpoints; unconfigured or lost Redis returns an explicit unavailable/stale state; cron only starts Workflow and requires `CRON_SECRET`; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run release:check` passed on 2026-08-19: Prisma validation/generation, lint, strict typecheck, 74 deterministic tests across 30 files, Workflow-enabled production build with 3 workflows/11 steps, 9 Playwright functional/security/accessibility/visual journeys, and 11 deterministic evaluations. `npm ci --dry-run --ignore-scripts` confirmed clean lockfile installation. `npm audit --omit=dev` remains limited to the documented Prisma `deepmerge-ts` advisory whose automated fix is a breaking downgrade. The labeled live replay remains precision 1.00 and recall 1.00.
