...
- **Current phase:** Phase 2 — Provider Adapters and Public Race Library. Phase 1 is complete; implementation now advances through the remaining approved plan without discretionary pauses.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized contracts, temporal and evidence invariants, canonical British/Dutch fixtures, repository ports, idempotent adapters/seeds, and live-verified core, timing, concept, explanation, media, connection, source, and external-reference persistence.
- **Active integrations:** Neon project `f1-watchcoach-development`, database `neondb`, main branch `br-divine-bonus-afev3o40`; Prisma 7.9.1. Applied migrations: `20260817173000_phase1_core`, `20260818193000_phase1_timing_evidence_types`, `20260818193100_phase1_timing_evidence`, and `20260818195500_phase1_learning_content`. No other hosted service is connected. Never record the connection string in this repository.
...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Tailwind CSS 4; Vitest; React Testing Library; Playwright 1.55.1; Zod at trust boundaries; Prisma uses its PostgreSQL driver adapter and generated client; canonical seed transactions have bounded Neon-aware limits (`maxWait` 30 seconds, `timeout` 120 seconds); manual SQL checks enforce evidence ranges, standing targets, media timestamps, non-self connections, and exactly one typed external-reference target; source joins use restrictive deletion while owned records cascade; PostgreSQL enum additions are committed separately before dependent constraints when required; protected media is referenced as metadata only; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run verify` passed on 2026-08-18 (Prisma validation/generation, lint, strict typecheck, 21 deterministic unit/component/schema tests, production build, and 1 Chromium Playwright test). Repeated live seeds completed successfully. `npm run db:verify:canonical` passed against Neon with exact core, timing, and learning-content counts; zero invalid relations, duplicates, self-connections, or orphaned provenance; and verified cascade/restrict policies. `npm audit --omit=dev` retains the known `deepmerge-ts@7.1.5` Prisma CLI/config advisory; the offered fix is a breaking Prisma downgrade.
- **Next checkpoint:** Implement fixture-backed Jolpica/OpenF1 adapters with typed failures, normalization, cache/deduplication, and public `/races` plus `/races/[season]/[round]` routes. Live-provider smoke tests remain opt-in.
