...
- **Current phase:** Phased MVP implementation complete (Phases 0–7), with the post-release universal Formula 1-only Watchcoach router, provider-backed race catalog, shared home/floating assistant, responsive motorsport timing visual system, and Google-only Clerk authentication guardrails integrated. Production is live at `https://f1-watchcoach.vercel.app` on the `RUBA101` Vercel team.
- **Visual-system decision:** On 2026-08-25 the frontend was overhauled without changing domain, provider, API, authentication, or persistence behavior. The accepted system uses near-black timing grids, white condensed display hierarchy, sparse racing-red structure, hairline rails, clipped controls, round/lap indices, and one continuous Watch → Learn → Connect spine. Existing factual copy and data remain authoritative; decorative telemetry is unlabeled and `aria-hidden`. Mobile keeps the floating racer assistant but reduces its footprint, and small accent text uses an AA-safe brighter red while structural red remains `#e10600`.
- **Visual verification:** The 2026-08-25 release-equivalent gate passed: Prisma validation/generation, lint, strict typecheck, 146 tests across 38 files, the Workflow-enabled production build, 11 Playwright journeys (including refreshed desktop/mobile visual baselines), and 18 deterministic evaluations across 6 files. Two browser-led review passes covered home, race library, curated/provider race detail, moment detail, learning-unavailable, live-unavailable, sign-in/unconfigured, not-found, and the open/answered floating assistant at 1440×1000 and 390×844. Final route renders had no horizontal overflow or browser errors; axe reported zero violations after list/group semantics were corrected, with translucent-background contrast verified manually at 18.5:1 primary, 8.28:1 muted, and 5.44:1 small accent text.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized contracts and live-verified Neon persistence. Phase 2 added normalized providers and public race browsing. Phase 3 delivered evidence-first Watch → Learn → Connect. Phase 4 added centralized grounded AI and Workflow boundaries. Phase 5 added optional Clerk identity, authorization-scoped learning memory, resume/preferences, and personalization. Phase 6 added secured live ingestion, Redis-compatible state, deterministic detection/persistence, replay evaluation, and read-only live views. Phase 7 completed on 2026-08-19 with expanded scenario coverage, request diagnostics, analytics/performance instrumentation, rate limiting, security/media/cache/bundle review, release configuration, and comprehensive README guidance.
- **Active integrations:** Vercel project `ruba-101/f1-watchcoach` (`prj_UfwTpVYXJvphgSj40wnKggFtbk3R`) serves production at `https://f1-watchcoach.vercel.app` with OpenAI and Google-only Clerk configuration stored in Vercel environments; database, Redis, and live-ingestion credentials remain unconfigured in production and degrade explicitly. Neon project `f1-watchcoach-development`, database `neondb`, main branch `br-divine-bonus-afev3o40`; Prisma 7.9.1; pgvector 0.8.6; Workflow 4.8.3; Clerk 7.7.8; Vercel Web Analytics and Speed Insights packages. Applied migrations: `20260817173000_phase1_core`, `20260818193000_phase1_timing_evidence_types`, `20260818193100_phase1_timing_evidence`, `20260818195500_phase1_learning_content`, `20260818221000_phase4_ai_metadata`, and `20260818234500_phase5_learning_memory`. Keyless Jolpica/OpenF1 historical reads are active; Jolpica supplies a normalized, paginated all-time driver directory, fully paginated driver career results and standings, and calendars/classifications. Optional OpenAI enables ambiguous scope-only classification, grounded race explanation, and trusted-domain Responses web retrieval; no separate search API is required. Never record connection strings or provider credentials in this repository.
- **Approved schema groups:** Phase 1 core, timing evidence, and learning-content groups plus Phase 4 AI metadata/embedding storage are implemented and live-verified. On 2026-08-18 the user explicitly authorized execution of the remaining phased plan without discretionary approval waits; platform-enforced migration confirmations, destructive-operation safeguards, and deployment permission remain mandatory.
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Zod at trust boundaries; Prisma/Neon as system of record; Redis REST only for ephemeral live state; authenticated mutations resolve Clerk identity to an internal `User`; all learning queries use user-scoped compound keys; live candidates use deterministic UUIDs and reuse `RaceMoment` types; AI may explain but never invent the event truth. Clerk exposes one fixed `oauth_google` entry point and callback, sends new and returning users to `/learning`, and returns sign-outs home. `/learning` uses server-side `auth.protect()` before user-data reads, while every mutation independently resolves server identity; public learning stays anonymous. The Clerk Dashboard must enable Google and disable every other strategy, and production Google OAuth credentials remain hosted configuration rather than repository data. The assistant now resolves known F1 entities/concepts deterministically, rejects confident non-F1 prompts before factual retrieval, produces a validated multi-intent plan, and retrieves only requested categories through one server-only orchestrator. Known driver aliases have bounded one-edit typo tolerance; drivers beyond the local table resolve through Jolpica's bounded/cached all-time directory before career retrieval. Driver credentials/“qualifications” route to career/profile evidence, while explicit qualifying/pole questions route to statistics. Unfamiliar person-name prompts may use the optional scope classifier instead of being rejected as non-F1 solely because the local alias table is incomplete. Structured race, career, standings, concept, moment, and media answers precede trusted-domain web search; a structured record that does not establish the requested causal/narrative detail is not treated as a final answer. Jolpica race rows directly calculate starts/wins/podiums/seasons/teams, while unsupported aggregates such as titles, poles, fastest laps, DNFs, and career points require cited retrieval. One bounded Responses web call can search the full allowlist while preferring primary domains, has a 45-second tool-aware timeout, and current/narrative/regulation questions fail closed when sourced evidence is unavailable. Web citations are URL/allowlist validated, only answer-attached citations are exposed when available, stable web results cache for 24 hours, current results for five minutes, and generated IDs/quotes/motives are never trusted. At most six browser-local conversation turns carry bounded resolved entity references and 800-character text excerpts for follow-ups; facts are re-retrieved each turn. Model instructions target at most 120 words, while every returned answer has a 1,600-character safety cap. The public Server Action retains its IP-hashed rate limit, and Next.js development Server Function argument logging is disabled to prevent prompt text in terminal logs. The global floating Watchcoach reuses the same panel and action; `/races` streams its static shell before provider-backed season cards so optional OpenF1 latency cannot block historical navigation, and remains Jolpica-authoritative with curated fallback. `F1_PROVIDER_MODE=fixtures` is an explicit CI/browser-test composition mode that fails provider ports closed into curated fallbacks; production and ordinary development default to live provider adapters. Provider fetch, normalize/cache, detect, persist, enrich, and publish remain distinct Workflow checkpoints; cron only starts Workflow and requires `CRON_SECRET`. The checked-in 00:00 UTC daily cron is compatible with the Vercel Hobby plan; race-frequency scheduling requires Pro or secured explicit triggers. Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** On 2026-08-27 the post-cron-change `npm run verify` passed locally: Prisma validation/generation, lint, strict typecheck, 152 deterministic tests across 40 files, the Workflow-enabled production build with 3 workflows/11 steps, and all 11 Playwright functional/security/accessibility/visual journeys. GitHub Actions Verify run `33094050369` passed the same deployed commit `293ceec`. Vercel preview `dpl_xMkxUq44qXsY47ybtgRQhUwcCbbd` reached READY and returned 200 for home, races, canonical moment, sign-in, and learning routes before production promotion. Production `dpl_y23iaEeiQzKYS9JSQG9bH8uVtiSc` reached READY; browser smoke confirmed the live provider-backed 2026 race directory, Google-only sign-in entry, and a grounded chatbot answer that Lewis Hamilton won the 2024 British GP. The post-smoke production error scan was clean. CI browser runs force deterministic provider fallbacks, serialize the two visual checks, compare fixed-size moment-hero regions, and keep separate macOS/Linux raster baselines. Failed workflows upload short-lived Playwright diagnostics. Secret/ignore checks confirm `.env.local`, `.vercel/`, and `.clerk/` are excluded. The combined provider smoke still records the external OpenF1 historical HTTP 401; optional timing coverage degrades explicitly. `npm audit --omit=dev` remains limited to the documented Prisma `deepmerge-ts` advisory, and labeled live replay remains precision 1.00/recall 1.00.
- **Next checkpoint:** Claim the Clerk instance, verify its Dashboard keeps Google enabled and every other strategy disabled, add the production domain to Clerk's allowed origins/redirects if required, and run the signed-in internal-user creation → save/resume → sign-out smoke with a disposable test account. Provision production Neon/Redis/live-ingestion credentials only when those hosted capabilities are intended, then apply reviewed production migrations before enabling persistence. Also investigate the external OpenF1 historical 401. Future deployments remain explicit user-approved operations.
- **Documentation:** `README.md` is the comprehensive setup, architecture, migration, provider, authentication, live-ingestion, observability, media-rights, deployment, command, and troubleshooting guide. Keep this ledger concise and update both files when architecture or operations change.
