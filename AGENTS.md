...
- **Current phase:** Phased MVP implementation complete (Phases 0–7), with the post-release universal Formula 1-only Watchcoach router, provider-backed race catalog, shared home/floating assistant, responsive motorsport timing visual system, and Google-only Clerk authentication guardrails integrated. Deployment remains intentionally pending explicit permission and hosted credentials.
...
- **Active integrations:** Neon project `f1-watchcoach-development`, database `neondb`, main branch `br-divine-bonus-afev3o40`; Prisma 7.9.1; pgvector 0.8.6; Workflow 4.8.3; Clerk 7.7.8 with a claimable keyless development instance and Google-only application flow; Vercel Web Analytics and Speed Insights packages. Applied migrations: `20260817173000_phase1_core`, `20260818193000_phase1_timing_evidence_types`, `20260818193100_phase1_timing_evidence`, `20260818195500_phase1_learning_content`, `20260818221000_phase4_ai_metadata`, and `20260818234500_phase5_learning_memory`. Keyless Jolpica/OpenF1 historical reads are active; Jolpica now supplies a normalized, paginated all-time driver directory, fully paginated driver career results and standings, and calendars/classifications. Optional OpenAI enables ambiguous scope-only classification, grounded race explanation, and trusted-domain Responses web retrieval; no separate search API is required. Clerk credentials stay in ignored `.env.local`; Redis, Workflow triggers, and live-session calls remain opt-in and absent from deterministic CI. Never record connection strings or provider credentials in this repository.
