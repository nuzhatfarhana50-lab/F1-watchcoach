...
- **Current phase:** Phase 6 — Live Race Ingestion and Moment Detection. Phases 0–5 are complete; implementation continues without discretionary pauses.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized domain contracts and live-verified Neon persistence. Phase 2 completed on 2026-08-18 with normalized providers and public race browsing. Phase 3 completed on 2026-08-18 with the evidence-first Watch → Learn → Connect experience. Phase 4 completed on 2026-08-18 with centralized grounded AI and Workflow boundaries. Phase 5 completed on 2026-08-19 with optional Clerk identity, internal users, authorization-scoped learning repositories, typed Server Actions, race/moment/concept memory, preferences, public fallback, and personalization regression coverage.
...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Tailwind CSS 4; Vitest; React Testing Library; Playwright 1.55.1; Zod at trust boundaries; Prisma uses its PostgreSQL driver adapter and generated client; public pages consume serializable read models through a server-only composition boundary; authenticated mutations resolve Clerk identity to an internal `User` and never accept a caller-supplied user ID; all learning queries use user-scoped compound keys; learning transitions permit only the same or next state; Clerk keys are optional but must be supplied as a pair; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run verify` passed on 2026-08-19: Prisma validation/generation, lint, strict typecheck, 60 deterministic tests across 24 files, Workflow-enabled production build with 2 workflows/5 steps, and 8 Playwright functional/security/accessibility/visual journeys. `npm run eval` passed 6 deterministic content/AI evaluations. Neon temporary-branch verification, parent promotion, Prisma checksum reconciliation, user isolation, idempotent encounter counting, and cascade cleanup all passed live.
- **Next checkpoint:** Add secured ingestion and cron Route Handlers, Redis-compatible live state, idempotent Workflow steps, deterministic replay fixtures, moment detection, retry/recovery semantics, and a read-only live session route.
