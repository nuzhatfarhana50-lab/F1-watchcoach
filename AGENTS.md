...
- **Current phase:** Phase 1 — Domain Contracts, Persistence Design, and Canonical Fixtures. The approved core schema is live in Neon; canonical database seeding is blocked on Prisma interactive-transaction startup timeout `P2028`.
...
- **Last verified commands:** `npm run verify` passed on 2026-08-17 (Prisma validation/generation, lint, strict typecheck, 12 deterministic unit/component/schema tests, production build, and 1 Chromium Playwright test). Live Neon inspection confirmed 15 approved tables, 27 foreign keys, 8 manual checks, and one completed Prisma migration record; failed seed attempts left zero race, moment, and source rows. `npm audit --omit=dev` reports the known `deepmerge-ts@7.1.5` advisory through Prisma's CLI/config path; the offered fix is a breaking Prisma downgrade.
- **Next checkpoint:** Resolve the canonical seed's repeated `P2028` failure to start its interactive transaction, then run it twice and verify live relations and stable counts. Pooled and direct Neon endpoints produced the same failure; investigate explicit Prisma transaction wait/timeout configuration before changing architecture. Later Phase 1 timing-evidence and concept/explanation/media schema groups require separate approval.
