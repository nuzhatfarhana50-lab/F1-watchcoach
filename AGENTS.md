...
- **Current phase:** Phase 7 — Hardening, Release Readiness, and Documentation. Phases 0–6 are complete; implementation continues without discretionary pauses.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized domain contracts and live-verified Neon persistence. Phase 2 completed on 2026-08-18 with normalized providers and public race browsing. Phase 3 completed on 2026-08-18 with the evidence-first Watch → Learn → Connect experience. Phase 4 completed on 2026-08-18 with centralized grounded AI and Workflow boundaries. Phase 5 completed on 2026-08-19 with optional Clerk identity and authorization-scoped learning memory. Phase 6 completed on 2026-08-19 with secured ingestion/cron boundaries, Redis-compatible live state, 6-step idempotent ingestion Workflow, selective PostgreSQL persistence, high-confidence moment detection, stale/cache-loss states, deterministic replay, and a read-only live route.
- **Active integrations:** Neon project `f1-watchcoach-development`, database `neondb`, main branch `br-divine-bonus-afev3o40`; Prisma 7.9.1; pgvector 0.8.6; Workflow 4.8.3; optional Clerk 7.7.8. Applied migrations: `20260817173000_phase1_core`, `20260818193000_phase1_timing_evidence_types`, `20260818193100_phase1_timing_evidence`, `20260818195500_phase1_learning_content`, `20260818221000_phase4_ai_metadata`, and `20260818234500_phase5_learning_memory`. Jolpica, OpenF1, and OpenAI production adapters are implemented; live calls remain opt-in and absent from deterministic CI. Never record connection strings or provider credentials in this repository.
...
- **Last verified commands:** `npm run verify` passed on 2026-08-19: Prisma validation/generation, lint, strict typecheck, 67 deterministic tests across 27 files, Workflow-enabled production build with 3 workflows/11 steps, and 9 Playwright journeys. `npm run eval` passed 7 evaluations. The labeled replay achieved precision 1.00 and recall 1.00; replays produced stable unique IDs and cache-loss/security degradation passed.
- **Next checkpoint:** Expand scenario fixtures, add production observability instrumentation and diagnostics, review security/caching/media/accessibility/bundle behavior, finish the production environment matrix and comprehensive README, then run clean-checkout-style release gates without deploying.
