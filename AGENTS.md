...
- **Current phase:** Phased MVP implementation complete (Phases 0–7), with post-release scoped race-question, provider-backed race-catalog, and site-wide floating Watchcoach assistant slices integrated. Deployment remains intentionally pending explicit permission and hosted credentials.
...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Zod at trust boundaries; Prisma/Neon as system of record; Redis REST only for ephemeral live state; authenticated mutations resolve Clerk identity to an internal `User`; all learning queries use user-scoped compound keys; live candidates use deterministic UUIDs and reuse `RaceMoment` types; AI may explain but never invent the event truth; the home race-question island rejects non-F1 prompts before retrieval, resolves canonical evidence before keyless Jolpica history, keeps result answers deterministic, validates generated citation IDs, and never becomes a general chatbot; its public Server Action is bounded by an ephemeral IP-hashed rate limit; the global floating Watchcoach reuses that exact panel and Server Action, keeps its browser-local conversation mounted while collapsed, and uses an original logo-free red/white open-wheel mascot asset; `/races` uses Jolpica as the authoritative season calendar, merges non-cancelled OpenF1 Grand Prix sessions from 2023 onward, preserves per-record provenance, and falls back only to verified curated races; Sprint sessions never count as Grands Prix; provider-only details show normalized classification without inventing learning moments; provider fetch, normalize/cache, detect, persist, enrich, and publish are distinct Workflow checkpoints; unconfigured or lost Redis returns an explicit unavailable/stale state; cron only starts Workflow and requires `CRON_SECRET`; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run release:check` passed on 2026-08-21 after the floating Watchcoach slice: Prisma validation/generation, lint, strict typecheck, 91 deterministic tests across 36 files, Workflow-enabled production build with 3 workflows/11 steps, 11 Playwright functional/security/accessibility/visual journeys, and 13 deterministic evaluations. Desktop/mobile browser inspection verified the collapsed mascot, responsive panel bounds, input autofocus, grounded 2024 British GP answer, non-F1 refusal, Escape dismissal/focus restoration, no horizontal overflow, meaningful content, and absence of framework overlays. `LIVE_F1_PROVIDER_TESTS=1 npm run test:integration:live -- tests/integration/providers.live.test.ts` previously passed against Jolpica/OpenF1; the corrected 2024 British OpenF1 session key is `9558`. `npm ci --dry-run --ignore-scripts` previously confirmed clean lockfile installation. `npm audit --omit=dev` remains limited to the documented Prisma `deepmerge-ts` advisory whose automated fix is a breaking downgrade. The labeled live replay remains precision 1.00 and recall 1.00.
- **Next checkpoint:** With explicit deployment permission, connect a Vercel project, provision the README environment matrix, verify Clerk/OpenAI/OpenF1/Redis hosted smoke suites on preview, review drains/runtime logs, apply reviewed production migrations, and promote the verified preview artifact. Do not deploy automatically.
