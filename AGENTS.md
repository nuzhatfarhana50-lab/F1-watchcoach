...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Zod at trust boundaries; Prisma/Neon as system of record; Redis REST only for ephemeral live state; authenticated mutations resolve Clerk identity to an internal `User`; all learning queries use user-scoped compound keys; live candidates use deterministic UUIDs and reuse `RaceMoment` types; AI may explain but never invent the event truth. The assistant now resolves known F1 entities/concepts deterministically, rejects confident non-F1 prompts before external work, produces a validated multi-intent plan, and retrieves only requested categories through one server-only orchestrator. Known driver aliases have bounded one-edit typo tolerance; driver credentials/“qualifications” route to career/profile evidence, while explicit qualifying/pole questions route to statistics. Unfamiliar person-name prompts may use the optional scope classifier instead of being rejected as non-F1 solely because the local alias table is incomplete. Structured race, career, standings, concept, moment, and media answers precede trusted-domain web search; current/narrative/regulation questions fail closed when sourced evidence is unavailable. Web citations are URL/allowlist validated, only answer-attached citations are exposed when available, stable web results cache for 24 hours, current results for five minutes, and generated IDs/quotes/motives are never trusted. At most six browser-local conversation turns carry bounded resolved entity references and 800-character text excerpts for follow-ups; facts are re-retrieved each turn. Model instructions target at most 120 words, while every returned answer has a 1,600-character safety cap. The public Server Action retains its IP-hashed rate limit, and Next.js development Server Function argument logging is disabled to prevent prompt text in terminal logs. The global floating Watchcoach reuses the same panel and action; `/races` remains Jolpica-authoritative with OpenF1 timing coverage and curated fallback. Provider fetch, normalize/cache, detect, persist, enrich, and publish remain distinct Workflow checkpoints; cron only starts Workflow and requires `CRON_SECRET`; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run release:check` passed on 2026-08-23 after the typo-tolerant driver routing and cited-evidence cleanup: Prisma validation/generation, lint, strict typecheck, 138 deterministic tests across 38 files, Workflow-enabled production build with 3 workflows/11 steps, 11 Playwright functional/security/accessibility/visual journeys, and 17 deterministic evaluations across 6 files. A live browser check of the exact reported prompt, `What are Max Verstapen's qualifications?`, resolved Max Verstappen and returned a concise sourced career answer with only its two answer-attached citations, HTTP 200, and no browser-console or framework-overlay errors. The earlier live four-turn check also verified follow-up entity resolution, complete Sainz team history through Williams, correct 2024 totals, and no 2,000-character validation error. The opt-in live Jolpica career/standings smoke passed with more than 100 Sainz results and Ferrari present. The combined provider smoke separately found OpenF1 HTTP 401 for a historical session request even though its current public documentation describes historical access as unauthenticated; the application continues to degrade that optional timing coverage explicitly. `npm ci --dry-run --ignore-scripts` previously confirmed clean lockfile installation. `npm audit --omit=dev` remains limited to the documented Prisma `deepmerge-ts` advisory whose automated fix is a breaking downgrade. The labeled live replay remains precision 1.00 and recall 1.00.
