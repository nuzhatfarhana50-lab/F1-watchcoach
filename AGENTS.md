...
- **Current phase:** Phase 5 — Authentication and Personal Learning Memory. Phases 0–4 are complete; implementation continues without discretionary pauses.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized domain contracts and live-verified Neon persistence. Phase 2 completed on 2026-08-18 with normalized providers and public race browsing. Phase 3 completed on 2026-08-18 with the evidence-first Watch → Learn → Connect experience. Phase 4 completed on 2026-08-18 with centralized OpenAI generation/embedding/classification/connection boundaries, Zod structured output, real-ID resolution, structured-first retrieval, curated fallback/cache behavior, durable Workflow steps, internal secured triggering, live pgvector/generation metadata persistence, and deterministic plus opt-in live evaluations.
- **Active integrations:** Neon project `f1-watchcoach-development`, database `neondb`, main branch `br-divine-bonus-afev3o40`; Prisma 7.9.1; pgvector 0.8.6; Workflow 4.8.3. Applied migrations: `20260817173000_phase1_core`, `20260818193000_phase1_timing_evidence_types`, `20260818193100_phase1_timing_evidence`, `20260818195500_phase1_learning_content`, and `20260818221000_phase4_ai_metadata`. Jolpica, OpenF1, and OpenAI production adapters are implemented; live calls remain opt-in and absent from deterministic CI. Never record connection strings or provider credentials in this repository.
...
- **Key implementation decisions:** Next.js 16 App Router; React 19; Node.js runtime; npm; strict TypeScript; Tailwind CSS 4; Vitest; React Testing Library; Playwright 1.55.1; Zod at trust boundaries; Prisma uses its PostgreSQL driver adapter and generated client; canonical seed transactions have bounded Neon-aware limits (`maxWait` 30 seconds, `timeout` 120 seconds); manual SQL checks enforce evidence ranges, standing targets, media timestamps, non-self connections, and exactly one typed external-reference target; source joins use restrictive deletion while owned records cascade; historical provider responses cache for 24 hours, recent metadata for five minutes, and live evidence for five seconds; concurrent identical provider requests share one in-flight promise; public pages consume serializable read models through a server-only composition boundary; moment UI presents structured evidence before explanation, labels missing telemetry rather than inferring it, resolves connection targets through repository records, and opens protected media only at attributed rights-holder URLs; visual baselines use a platform-neutral path with a bounded pixel-difference threshold; Webpack production builds because Turbopack's CSS worker cannot bind its internal port in the managed macOS environment.
- **Last verified commands:** `npm run verify` passed on 2026-08-18 (Prisma validation/generation, lint, strict typecheck, 52 deterministic tests across 21 files, Workflow-enabled production build with 2 workflows/5 steps, 5 Chromium functional/security/accessibility journeys, and 2 visual regressions). `npm run eval` passed 6 deterministic content/AI evaluations. Neon staging/promotion, Prisma checksum reconciliation, vector constraints, generation/source relations, idempotency, and cleanup all passed live. `npm audit --omit=dev` is back to the pre-existing Prisma `deepmerge-ts` advisory only; scoped nanoid/undici overrides remove Workflow transitive advisories.
- **Next checkpoint:** Integrate Clerk while preserving public browsing; add the internal user and learning-memory schema group, authorization-isolated repositories, typed Server Actions, resume state, preferences, and personalization regression coverage.
- **Documentation:** `README.md` documents local setup, safe database workflows, provider boundaries and freshness, fixture strategy, public routes, and verification; continue expanding it as later phases land.
