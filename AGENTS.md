...
- **Current phase:** Phase 4 — Grounded AI, Retrieval, and Connection Evaluation. Phases 0–3 are complete; implementation continues without discretionary pauses.
- **Completed gates:** Phase 0 completed on 2026-08-17. Phase 1 completed on 2026-08-18 with normalized domain contracts and live-verified Neon persistence. Phase 2 completed on 2026-08-18 with normalized Jolpica/OpenF1 adapters and public race browsing. Phase 3 completed on 2026-08-18 with evidence-first moment detail, curated Watch → Learn → Connect explanations, attributed rights-holder media references, explicit real-moment connections, source tracing, partial-data states, accessibility coverage, and desktop/mobile visual regression.
- **Active integrations:** Neon project `f1-watchcoach-development`, database `neondb`, main branch `br-divine-bonus-afev3o40`; Prisma 7.9.1. Applied migrations: `20260817173000_phase1_core`, `20260818193000_phase1_timing_evidence_types`, `20260818193100_phase1_timing_evidence`, and `20260818195500_phase1_learning_content`. Jolpica and OpenF1 production adapters are implemented but live calls are not used by deterministic CI or the default UI composition. No other hosted service is connected. Never record connection strings or provider credentials in this repository.
- **Approved schema groups:** Phase 1 core, timing evidence, and learning-content groups are implemented. On 2026-08-18 the user explicitly authorized execution of the remaining phased plan without discretionary approval waits; platform-enforced migration confirmations, destructive-operation safeguards, and deployment permission remain mandatory.
...
- **Last verified commands:** `npm run verify` passed on 2026-08-18 (Prisma validation/generation, lint, strict typecheck, 39 deterministic tests across 15 files, production build, 4 Chromium functional/accessibility journeys, and 2 responsive visual regressions). `npm run eval` separately passed 3 curated-content grounding checks. Live-provider smoke tests remain outside CI. Repeated live seeds and `npm run db:verify:canonical` remain green against Neon. `npm audit --omit=dev` retains the known `deepmerge-ts@7.1.5` Prisma CLI/config advisory; the offered fix is a breaking Prisma downgrade.
- **Next checkpoint:** Centralize OpenAI generation and retrieval ports, add Zod-validated structured outputs and real-ID resolution, introduce schema-approved generation/embedding metadata, and build deterministic plus opt-in live-model evaluations with curated fallback behavior.
