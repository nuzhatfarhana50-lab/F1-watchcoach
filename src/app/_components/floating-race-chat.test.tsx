...
describe("FloatingRaceChat", () => {
  it("opens from the mascot, reuses the scoped chat, and closes with Escape", async () => {
    ...
    const trigger = screen.getByRole("button", { name: "Open Watchcoach race assistant" });
    const panel = screen.getByTestId("floating-race-chat-panel");
    expect(panel).not.toBeVisible();

    await user.click(trigger);
    expect(panel).toBeVisible();
    await waitFor(() => expect(screen.getByLabelText("Ask a race question")).toHaveFocus());
    ...
    expect(panel).not.toBeVisible();
    await waitFor(() => expect(screen.getByRole("button", { name: "Open Watchcoach race assistant" })).toHaveFocus());
  });
});
