...
describe("LiveSessionService", () => {
  it("publishes and reads the normalized session contract", async () => {
    ...
    const moments = detectMoments(9558, britishReplayEvidence);
    const published = await service.publish(9558, britishReplayEvidence, moments, now);
    const result = await service.read(9558, new Date("2024-07-07T15:10:10.000Z"));
  ...
  it("marks an expired checkpoint stale without corrupting it", async () => {
    const service = new LiveSessionService(new InMemoryLiveStateCache());
    await service.publish(9558, britishReplayEvidence, [], new Date());
    const result = await service.read(9558, new Date(Date.now() + 21_000));
  ...
  it("returns an explicit unavailable result after cache loss", async () => {
    ...
    await expect(service.read(9558)).resolves.toEqual({ kind: "unavailable", reason: "cache-unavailable" });
  });
});
